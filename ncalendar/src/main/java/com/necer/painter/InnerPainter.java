...
public class InnerPainter implements CalendarPainter {
    ...
    @Override
    public void onDrawToday(Canvas canvas, RectF rectF, LocalDate localDate, List<LocalDate> selectDateList,int flag) {
        if (selectDateList.contains(localDate)) {
            ...
            drawPoint(canvas, rectF, true, noAlphaColor, localDate,flag);
        } else {
            drawSolar(canvas, rectF, localDate, noAlphaColor, false, true);
            drawPoint(canvas, rectF, false, noAlphaColor, localDate,flag);
        }
        drawStretchText(canvas, rectF, noAlphaColor, localDate);
    }

    @Override
    public void onDrawCurrentMonthOrWeek(Canvas canvas, RectF rectF, LocalDate localDate, List<LocalDate> selectDateList,int flag) {
        if (selectDateList.contains(localDate)) {
    ...
    @Override
    public void onDrawLastOrNextMonth(Canvas canvas, RectF rectF, LocalDate localDate, List<LocalDate> selectDateList,int flag) {
        if (selectDateList.contains(localDate)) {
            ...
            drawPoint(canvas, rectF, false, mAttrs.alphaColor, localDate,flag);
    ...
    public void onDrawDisableDate(Canvas canvas, RectF rectF, LocalDate localDate) {
        ...
        drawPoint(canvas, rectF, false, mAttrs.disabledAlphaColor, localDate,0);
    ...
    private void drawSelectBg(Canvas canvas, RectF rectF, int alphaColor, boolean isToday) {
        ...
        colors[0] = Color.parseColor("#8820c3ff");
        colors[1] = Color.parseColor("#4d64f9");
        LinearGradient linearGradient = new LinearGradient(rectF.left, rectF.top+(rectF.height()-rectF.width())/2,
                rectF.right, rectF.bottom, colors, null, Shader.TileMode.CLAMP);
        ...
        canvas.drawRoundRect(new RectF(rectF.left,rectF.top+(rectF.height()-rectF.width())/2,rectF.right,
                rectF.bottom-(rectF.height()-rectF.width())/2),40,40,mCirclePaint);
    }
    ...
    private void drawPoint(Canvas canvas, RectF rectF, boolean isTodaySelect, int alphaColor, LocalDate date,int flag) {
        ...
        if (pointList.contains(date)) {
            ...
            int []colors = new int[2];
            colors[0] = Color.parseColor("#f8da3b");
            colors[1] = Color.parseColor("#ff8059");
            LinearGradient linearGradient = new LinearGradient(rectF.centerX()-mAttrs.pointSize/2, rectF.centerY()+ mAttrs.pointDistance-mAttrs.pointSize/2,
                    rectF.centerX()+mAttrs.pointSize/2, rectF.centerY()+ mAttrs.pointDistance+mAttrs.pointSize/2, colors, null, Shader.TileMode.CLAMP);
            mPointPaint.setShader(linearGradient);
            mPointPaint.setStyle(Paint.Style.FILL);
            mPointPaint.setColor(isTodaySelect ? mAttrs.todaySelectContrastColor : mAttrs.pointColor);
            mPointPaint.setAlpha(alphaColor);
            canvas.drawCircle(rectF.centerX(),rectF.centerY() + mAttrs.pointDistance, mAttrs.pointSize, mPointPaint);
        }
    }
