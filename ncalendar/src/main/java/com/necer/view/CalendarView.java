...
public class CalendarView extends View implements ICalendarView {
    ...
    public CalendarView(Context context, BaseCalendar calendar, LocalDate initialDate, CalendarType calendarType,int flag) {
        super(context);
        this.flag = flag;
        mCalendarHelper = new CalendarHelper(calendar, initialDate, calendarType);
        mDateList = mCalendarHelper.getDateList();
                ...
                if (mCalendarHelper.isAvailableDate(localDate)) { //可用的日期
                    if (mCalendarHelper.isCurrentMonthOrWeek(localDate)) {  //当月日期
                        if (CalendarUtil.isToday(localDate)) {  //当天
                            calendarPainter.onDrawToday(canvas, rectF, localDate, mCalendarHelper.getAllSelectListDate(),flag);
                        } else { //非当天的当月日期
                            calendarPainter.onDrawCurrentMonthOrWeek(canvas, rectF, localDate, mCalendarHelper.getAllSelectListDate(),flag);
                        }
                    } else { //上下月日期
                        calendarPainter.onDrawLastOrNextMonth(canvas, rectF, localDate, mCalendarHelper.getAllSelectListDate(),flag);
                    }
                } else { //不可用日期
